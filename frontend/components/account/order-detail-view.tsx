...
/**
 * Renders exactly the `OrderDetails` the server already resolved and
 * ownership-checked (see app/[locale]/account/orders/[id]/page.tsx) —
 * nothing here re-fetches, re-derives ownership, or trusts anything
 * from the URL beyond what already produced this `order` prop. Reuses
 * the Phase 1 status model as-is via OrderStatusBadge/OrderStatusTimeline
 * rather than re-implementing any status logic.
 */
export function OrderDetailView({ order }: OrderDetailViewProps) {
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="flex flex-col gap-4 rounded-(--radius-card) border border-border bg-surface p-6">
        ...
        <dl className="flex flex-col gap-2 text-body-s">
          ...
          {order.discountTotal > 0 && (
            <div className="flex justify-between">
              <dt className="text-text-muted">{ta("discountLabel")}</dt>
              <dd className="text-error">-<Price amount={order.discountTotal} currency={order.currency} /></dd>
            </div>
          )}
      ...
      {(order.customerName || order.shippingAddress1 || order.shippingCity || order.customerPhone) && (
        <div className="flex flex-col gap-4 rounded-(--radius-card) border border-border bg-surface p-6">
      ...
      <Button variant="secondary" href="/account/orders" className="w-full rounded-xl py-3.5">
        {ta("backToOrders")}
      </Button>
    </div>
  );
